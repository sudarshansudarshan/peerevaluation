  ...
  try {
    ...
    if (batchExists) {
      return res
        .status(400)
        .json({
          message: "Batch with this ID already exists for the selected course!",
        });
    }
    ...
    if (batchId !== existingBatch.batchId) {
      ...
      if (batchIdExists) {
        return res
          ...
          .json({
            message:
              "Batch with this ID already exists for the selected course!",
          });
      }
    }
...
export const getDashboardCounts = async (req, res) => {
  try {
    const [teacherCount, courseCount, batchCount, studentCount] =
      await Promise.all([
...
export const deleteCourse = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Course and associated batches deleted successfully!" });
  } catch (error) {
    res.status(500).json({ message: "Server error. Please try again later!" });
  }
};
