...
export const getStudentDashboardStats = async (req, res) => {
  try {
    ...
    const pendingEvaluations = evaluations.filter(
      (evaluation) => evaluation.exam && evaluation.exam.flags === false
    ).length;
...
export const requestEnrollment = async (req, res) => {
  try {
    ...
    if (existingEnrollment && existingEnrollment.status === "active") {
      ...
      return res
        .status(400)
        .json({ message: "You have been dropped from this batch!" });
    }
      ...
      completedExams.map(async (exam) => {
        ...
        if (totalEvaluations > 0) {
          const evaluationTotals = evaluations.map((evaluation) => {
            const totalMarksForEvaluation = evaluation.score.reduce(
              (sum, mark) => sum + mark,
              0
            );
          ...
          const sumOfAllEvaluations = evaluationTotals.reduce(
            (sum, total) => sum + total,
            0
          );
        ...
        examObj.percentage =
          exam.totalMarks > 0
            ? Math.round((aggregateMarks / exam.totalMarks) * 100 * 100) / 100
            : 0;
...
export const uploadExamDocument = async (req, res) => {
  ...
  try {
    ...
    if (
      existingDoc &&
      existingDoc.uploadedBy.toString() === studentId.toString()
    ) {
      ...
      return res
        .status(200)
        .json({ message: "Existing document updated successfully!" });
    } else if (
      existingDoc &&
      existingDoc.uploadedBy.toString() !== studentId.toString()
    ) {
  ...
  try {
    ...
    for (const evaluation of evaluations) {
      ...
      if (exam && exam.batch && !batchCourseMap[exam._id]) {
        const batch = await Batch.findOne({ _id: exam.batch }).populate(
          "course",
          "courseName"
        );
        if (batch) {
...
export const raiseTicket = async (req, res) => {
  try {
    ...
    if (evaluation.ticket === 1) {
      return res
        .status(400)
        .json({ message: "Ticket already raised for this evaluation!" });
    }
